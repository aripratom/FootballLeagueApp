...
class ScheduleAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.bindItem(schedule[position], gson, apiRepository)
    }


    class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        @SuppressLint("SetTextI18n")
        fun bindItem(schedule: ScheduleItem, gson: Gson, apiRepository: ApiRepository) {
            ...
            GlobalScope.launch(Dispatchers.Main) {
                val data = gson.fromJson(
                    apiRepository.doRequest(TheSportDBApi.getTeamLogo(schedule.homeTeamID)).await(),
            ...
            GlobalScope.launch(Dispatchers.Main) {
                val data = gson.fromJson(
                    apiRepository.doRequest(TheSportDBApi.getTeamLogo(schedule.awayTeamID)).await(),
                    TeamResponse::class.java
                )
